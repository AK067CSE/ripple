"""Hybrid vector store that works with or without ML dependencies."""
...
# Try to import ML dependencies, fall back to simple mode if not available
try:
...
class HybridVectorStore:
    ...
    def add_query_embedding(self, query: str, intent: Optional[str] = None) -> int:
        ...
        global ML_AVAILABLE
        if ML_AVAILABLE and self.faiss_index is not None:
            try:
    ...
    def find_similar_queries(
        ...
        global ML_AVAILABLE
        if ML_AVAILABLE and self.faiss_index is not None and self.faiss_index.ntotal > 0:
            try:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'ml_mode': ML_AVAILABLE and self.model is not None,
